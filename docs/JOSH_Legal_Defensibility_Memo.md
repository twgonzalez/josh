...
The JOSH system computes a single number for each proposed development: **how many minutes the project adds to evacuation clearance time on its most constrained serving route.** This number — ΔT — is derived entirely from published national engineering standards (HCM 2022, 7th Edition), federal data (Census ACS), state hazard designations (Cal Fire FHSZ), national community-evacuation standards (NFPA 1660:2024 / NFPA 1616:2020), the national life-safety code for building egress (NFPA 101, 2024 California edition), federal post-disaster case studies (NIST Camp Fire investigation, TN 2135), and the 2025 California Wildland-Urban Interface Code (CWUIC) adopted by the State Fire Marshal. No input requires city-level policy adoption. No computation involves discretion.
...
The architecture is simple. FHSZ affects the road — fire zones reduce road capacity during the emergency that triggers evacuation. Mobilization is a constant — the standard assumes 90% of the project's households generate a vehicle, consistent with the community mass-evacuation design framework in NFPA 1660:2024 (consolidating NFPA 1616:2020), adjusted from full evacuation for the ~10% zero-vehicle household share documented by Census ACS B25044. The calculation is one division: project vehicles divided by bottleneck capacity, converted to minutes.
...
Where:
  project_vehicles    = units × vehicles_per_unit × 0.90
                        (0.90 = NFPA 1660:2024 / NFPA 1616:2020 community mass-evacuation design basis;
                         full evacuation adjusted for ~10% zero-vehicle HHs per Census ACS B25044)
  bottleneck_capacity = min(HCM_capacity × hazard_degradation) along serving route
                        (HCM_capacity = HCM 2022, 7th Edition, Transportation Research Board;
                         hazard_degradation = composite engineering-judgment factor, independent
                         traffic-engineering review pending per Fire Science Consulting LLC, May 2026)
  egress_penalty      = building egress time for structures ≥ 4 stories
                        (NFPA 101 Life Safety Code, 2024 California edition, Ch. 7;
                         IBC 2024 Ch. 10 — Means of Egress)
```
...
2. Housing units generate vehicles (Census ACS B25044).
3. 90% of households evacuate by vehicle during an emergency (NFPA 1660:2024 / NFPA 1616:2020 community mass-evacuation design basis, adjusted from full evacuation for ~10% zero-vehicle households per Census B25044).
4. The road has a fixed capacity, potentially reduced if it passes through a fire hazard zone (HCM 2022, 7th Edition × composite WUI hazard-degradation factor applied to Cal Fire FHSZ segments).
...
**Road capacity:** Highway Capacity Manual, 7th Edition (HCM 2022). Published by the Transportation Research Board, a unit of the National Academies of Sciences, Engineering, and Medicine. Used by every state DOT and traffic engineering practice in the country. Specific exhibits: 12-6 (freeway base capacity), 12-7 (multilane base capacity), Chapter 15 (two-lane highways).

**Capacity degradation (FHSZ road adjustment):** Composite engineering-judgment factor; underlying derivation is subject to independent traffic-engineering review (Fire Science Consulting LLC, Ziazi & Simeoni, May 2026; open item on the JOSH Methodology Roadmap). The factor scaling is conservative relative to HCM 2022 Chapter 11 Exhibit 11-20 worst-case weather Capacity Adjustment Factors on the basis that WUI fire conditions (smoke visibility, debris, counterflow evacuation traffic, and concurrent emergency apparatus access per CCR 1273.00) impose constraints more severe than any HCM-calibrated weather scenario. Empirical validation references: NIST TN 2135 (Maranghides et al. 2021, Camp Fire timeline reconstruction; spot fires reached Paradise approximately 40 minutes ahead of the fire front, with documented road burnovers on 3 of 5 evacuation routes); Rohaert et al. (2023), Transp. Res. Part D 116:103424 (Kincade Fire traffic dynamics); Wetterberg et al. (2022), Safety Science (smoke effects on driving speed). Earlier JOSH documentation that cited HCM Exhibits 10-15 (Lane Closure Severity Index for work zones) and 10-17 (a photograph) as the source of these factors was imprecise and has been corrected: those exhibits contain no fire, smoke, or visibility capacity values. The composite scaling and FHSZ-segment trigger are unchanged; only the attribution is corrected. The FHSZ designation remains the sole determinant of which road segments receive degradation — it is a state finding under Gov. Code §51175, not a city opinion.

**Mobilization rate (0.90):** NFPA 1660:2024 (*Standard for Emergency, Continuity, and Crisis Management: Preparedness, Response, and Recovery*), which consolidates NFPA 1616:2020 (*Standard on Mass Evacuation, Sheltering, and Re-entry Programs*), is the national fire-protection standard governing community-scale mass evacuation planning. It is the appropriate citation for the JOSH vehicle-generation rate. The 0.90 factor is derived from the standard's full-evacuation design basis, adjusted for the approximately 10% of households with zero vehicles as measured by Census ACS Table B25044. The mobilization rate is a constant — it does not vary by hazard zone, because the standard measures the project's demand on the road, not observed behavioral patterns during past events. As supporting reasoning by analogy: NFPA 101 (Life Safety Code, 2024 California edition), the national standard for *building* egress design, is premised on 100% occupant evacuation — fire marshals do not size exits for partial evacuation, they size them for everyone. The same full-load design principle is here applied at the community scale, with NFPA 1660 / 1616 as the operative source rather than NFPA 101 (which governs pedestrian egress inside buildings, not vehicles on roads). Earlier JOSH documentation that cited NFPA 101 as the operative source for the mobilization rate was imprecise and has been corrected (see Fire Science Consulting LLC review, May 2026); NFPA 101 is retained as analogical reasoning and as the operative source for the high-rise building-egress penalty only. Empirical California validation for the 0.90 magnitude includes Roberson, B.S., Peterson, D., and Parsons, R.W. (2012), *Attitudes on wildfire evacuation* (J. Emergency Management 10(5), 335-347).
...
**Building egress time:** NFPA 101 (Life Safety Code, 2024 California edition, Ch. 7) and International Building Code (IBC 2024, Ch. 10 — Means of Egress). National standards adopted by reference in the California Building Code. This is the legitimate use of NFPA 101 in the JOSH methodology: the 1.5-minute-per-story egress penalty for buildings of four stories or more, capped at 12 minutes, reflects the NFPA 101 stair-descent design rate for full occupant load plus the IBC 2024 Ch. 10 garage-egress allowance.
...
### 3.7 Scope and Limitations — CWUIC 2025 Acknowledgment

This subsection responds to the May 2026 independent technical review by Fire Science Consulting LLC (Ziazi & Simeoni) and aligns the methodology with the operative California Wildland-Urban Interface Code adopted by the State Fire Marshal in 2025.

**Civilian outbound capacity only.** JOSH measures the ΔT contribution of civilian vehicles exiting the project on its serving evacuation routes. The methodology does not reduce bottleneck capacity to account for concurrent inbound emergency apparatus access. Concurrent civilian/apparatus access is required by CCR 1273.00 of the **2025 California Wildland-Urban Interface Code (CWUIC)** — the operative California WUI code adopted by the State Fire Marshal, superseding CFC Chapter 49 and CBC Chapter 7A. A separate analysis is required to demonstrate compliance with CCR 1273.00 for projects on roads where simultaneous inbound apparatus access during a fire is not assured. Cities applying the JOSH ΔT standard should require this separate analysis as a condition of approval for any project on a constrained road in a designated fire hazard zone.

**Screening tool, not standalone denial instrument.** Consistent with **CWUIC Appendix C §C101.6** (which presents the NIST Minimum Throughput Time approach as an *initial idealized order-of-magnitude clearance estimate* but expressly states it is not a substitute for a full community evacuation study), the JOSH ΔT methodology is positioned as a screening tool that triggers further review under the Housing Accountability Act safety exception (Gov. Code §65589.5(j)(1)). A ΔT exceedance is the basis for *discretionary review*, not automatic denial. The discretionary tier preserves the council's deliberative authority to make a §65589.5(j)(1) finding on the full record, which may include a project-specific community evacuation study commissioned by the applicant.

**Open methodology items.** The May 2026 Fire Science Consulting review identified the following items as conditions on the methodology's full scientific defensibility. Each is on the JOSH Methodology Roadmap with target resolution dates:

1. **Hazard-degradation factor derivation.** Empirical re-derivation of the 0.35 / 0.50 / 0.75 factors from one or more of: (a) HCM 2022 Chapter 11 Exhibit 11-20 worst-case weather Capacity Adjustment Factors with documented compositing methodology and justification for why WUI fire conditions are more severe than HCM heavy-snow; (b) Rohaert et al. (2023) Kincade Fire and Wetterberg et al. (2022) smoke-visibility empirical traffic data; or (c) an independent commissioned traffic-engineering study.
2. **CWUIC road-geometry pre-check (Standard 6).** Validation of road geometry against CWUIC §§403.1 and 403.3 minimums (20-foot single-direction, 26-foot two-way) before applying HCM base capacity, so that projects on non-CWUIC-compliant roads receive a geometry flag rather than an HCM-base optimistic capacity. The `road_overrides.yaml` infrastructure for `width_ft` and `access_type` is already in place.
3. **Shadow-evacuation and background traffic loading.** Either a background traffic loading factor (Zhao et al. 2022, Wu et al. 2022, Rohaert et al. 2023) added to the bottleneck capacity calculation, or an explicit scope statement that ΔT measures the project's marginal contribution assuming a clear road.
4. **Cumulative route capacity register.** Route-level cumulative ΔT tracking across multiple individually compliant projects, so that the 5% per-project share functions as a population-level safety standard rather than a per-project screen that can be individually satisfied while collectively exhausted.
5. **δt(egress) for low-rise.** Formalization of total clearance time as `ΔT(road) + δt(egress)(building)` using IBC 2024 Ch. 10 flow rates and SFPE Handbook 5th Ed. Ch. 64 (Movement of People) pre-evacuation data, framed within the Required Safe Egress Time (RSET) concept (CFPA-E Guideline No. 19). The current methodology defines δt(egress) for buildings ≥ 4 stories only.
6. **NFPA 1660 (2024) / EMAP framing.** Explicit citation of NFPA 1660:2024 as the current consolidated mass-evacuation standard and reference to the Emergency Management Accreditation Program (EMAP) framework used by California local emergency managers, so JOSH determinations are positioned as consistent with EMAP-accredited evacuation planning practice.

The structural framework of the JOSH methodology — bottleneck capacity divided by vehicle demand, the use of HCM Chapter 15 for two-lane road capacity, FHSZ as the policy trigger, Census B25044 vehicle-data sourcing, and the deterministic audit trail — was independently confirmed as sound in the May 2026 review. The open items above are refinements to be implemented on a documented schedule; they are not grounds to delay current applications under the methodology.
...
- NFPA 1660:2024 (*Standard for Emergency, Continuity, and Crisis Management*, consolidating NFPA 1616:2020 mass-evacuation framework) is the national fire-protection standard governing community-scale mass evacuation. The Council adopts the standard's full-evacuation design basis at the community scale, with a 90% vehicle generation rate accounting for the ~10% zero-vehicle household share in Census ACS B25044. The Council notes that the same full-load design principle has long been applied at the building scale by NFPA 101 (Life Safety Code), and finds that applying it at the road scale is the natural extension of established fire-protection engineering practice.
...
- Specify the exact formula: `ΔT = (units × vehicles_per_unit × 0.90) / bottleneck_capacity × 60 + egress_penalty`, where bottleneck_capacity is the minimum effective capacity (HCM 2022 capacity × composite hazard-degradation factor for the road segment's FHSZ zone) along the project's serving evacuation routes. The 0.90 mobilization rate is a constant applied to all projects regardless of location, sourced from **NFPA 1660:2024 / NFPA 1616:2020 community mass-evacuation design framework**, adjusted from the full-evacuation design basis for the ~10% zero-vehicle household share in Census ACS B25044, and empirically validated against Roberson et al. (2012) California WUI stated-intent literature.
- Specify the composite hazard-degradation factors by FHSZ zone (VHFHSZ: 0.35, High: 0.50, Moderate: 0.75, Non-FHSZ: 1.00). These factors are conservative engineering-judgment values anchored against the HCM 2022 Chapter 11 weather Capacity Adjustment Factor framework (Exhibit 11-20) and validated against NIST TN 2135 Camp Fire empirical observations; the underlying derivation is subject to independent traffic-engineering review (Fire Science Consulting LLC review of May 2026, on the JOSH Methodology Roadmap). The factors affect road capacity only — not mobilization.
- Specify the safe egress windows by zone (VHFHSZ: 45 min, High: 90 min, Moderate: 120 min, Non-FHSZ: 120 min) with citation to NIST Technical Note 2135 (Maranghides et al. 2021). The 45-minute VHFHSZ figure is derived from the TN 2135 Supplement Appendix C reconstruction of the Camp Fire timeline (spot fires reached Paradise at approximately 07:50; fire front arrived at approximately 08:30 — 40 minutes — plus 5 minutes for modern Wireless Emergency Alert systems not available in 2018); explicit sourcing from the main report is on the Methodology Roadmap.
...
- Specify the egress penalty methodology: 1.5 minutes per story for buildings ≥ 4 stories, capped at 12 minutes, per NFPA 101 (Life Safety Code, 2024 California edition, Ch. 7) and IBC 2024 Ch. 10 (Means of Egress). This is the legitimate use of NFPA 101 in the JOSH methodology — building egress, not community evacuation mobilization. The penalty may be overridden by a project-specific NFPA 101 egress calculation prepared by a licensed fire protection engineer.
- Specify scope and limitations: ΔT measures civilian outbound capacity only. Concurrent emergency apparatus inbound access per CCR 1273.00 of the 2025 California Wildland-Urban Interface Code (CWUIC) requires separate analysis; the determination shall require this separate analysis as a condition of approval for projects on constrained roads in designated fire hazard zones. Consistent with CWUIC Appendix C §C101.6, the ΔT output is a screening tool that triggers discretionary review under §65589.5(j)(1), not a standalone permit-denial instrument.
...
Specifically: if a developer challenges the ΔT thresholds (e.g., "2.25 minutes in VHFHSZ is too restrictive"), the court's question under resolution-level review is: "Was it reasonably debatable that 2.25 minutes is an appropriate threshold for this hazard zone?" Given the underlying data — NFPA 1660 / 1616 community mass-evacuation mobilization design basis, HCM 2022 capacity figures, NIST-documented fire timelines, Camp Fire and Palisades Fire outcomes — the answer is clearly yes. The council reviewed published evidence and made a policy judgment within the range of reasonable safety standards. That judgment is entitled to deference.
...
Response: Observed evacuation rates from GPS studies (Zhao et al. 2022, Wong et al. 2020) show approximately 47% compliance during the Kincade Fire. However, observed rates measure behavioral patterns during a specific past event — they are not a design standard. **NFPA 1660:2024 (consolidating NFPA 1616:2020), the national fire-protection standard governing community-scale mass evacuation, applies a full-evacuation design basis to community-scale mass evacuation programs.** Emergency managers and fire authorities do not plan capacity based on the assumption that half the people will decide to stay. They plan for the case where everyone needs to get out, because that is the case that kills people if the road cannot handle the demand.

The 90% mobilization rate applies this design principle to the road. It asks: if this community needs to be evacuated, can the road handle the vehicles? The 10% reduction from 100% accounts for households with no vehicle (Census ACS B25044). The standard is designed for the emergency that requires evacuation, not for the average case where many people choose to stay. The same full-load design principle is well-established at the building scale through NFPA 101 (Life Safety Code), where fire marshals size stairwells and exits for 100% occupant evacuation; NFPA 1660 / 1616 extends that principle to the community scale, which is the principle JOSH applies. Empirical California validation for the 0.90 magnitude is provided by Roberson et al. (2012), *Attitudes on wildfire evacuation* (J. Emergency Management 10(5), 335-347), which documents stated-intent evacuation rates in Southern California WUI communities consistent with the NFPA 1660 / 1616 design basis adjusted for zero-vehicle households.
...
**For the judge:** The city adopted an objective standard — based on national fire-protection community mass-evacuation standards (NFPA 1660 / 1616), the national life-safety code for building egress (NFPA 101), highway capacity standards (HCM 2022), and the state's own fire hazard designations (Cal Fire FHSZ) — that measures how many minutes each project adds to evacuation time, and this project exceeds the threshold derived from the NIST-documented escape window for its hazard zone.

**For the record:** Pursuant to Resolution No. [XX-XXXX], the project adds [X] minutes to evacuation clearance on [route] — computed from [N] units × 1.9 vehicles (Census ACS B25044) × 90% community mass-evacuation mobilization rate (NFPA 1660:2024 / NFPA 1616:2020 design basis, adjusted from full evacuation for ~10% zero-vehicle households per Census B25044; empirical California validation per Roberson et al. 2012) divided by [C] vehicles per hour (HCM 2022, 7th Edition × composite hazard-degradation factor on Cal Fire FHSZ-designated segments) — exceeding the adopted threshold of [T] minutes, which is 5% of the [W]-minute safe egress window documented by NIST Technical Note 2135. This determination measures civilian outbound capacity only; concurrent emergency apparatus access per CCR 1273.00 of the 2025 California Wildland-Urban Interface Code requires separate analysis as a condition of any approval.
