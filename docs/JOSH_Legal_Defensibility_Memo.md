...
- Specify the exact formula: `ΔT = (units × vehicles_per_unit × 0.90) / bottleneck_capacity × 60 + egress_penalty`, where bottleneck_capacity is the minimum effective capacity (HCM capacity × hazard degradation factor for the road segment's FHSZ zone) along the project's serving evacuation routes. The behavioral mobilization rate of 0.90 is a constant applied to all projects regardless of location, sourced from FHWA Emergency Transportation Operations (mandatory evacuation compliance rate for residential areas).
...
**For the record:** Pursuant to Resolution No. [XX-XXXX], the project adds [X] minutes to evacuation clearance on [route] — computed from [N] units × 1.9 vehicles (Census ACS B25044) × 90% behavioral mobilization (FHWA Emergency Transportation Operations) divided by [C] vehicles per hour (HCM 2022 capacity × Cal Fire FHSZ degradation) — exceeding the adopted threshold of [T] minutes, which is 5% of the [W]-minute safe egress window documented by NIST Technical Note 2135.
