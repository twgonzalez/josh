...
| 6 | ~~**4-function `HazardAdapter` interface**~~ **SUPERSEDED by #12 on 2026-05-24.** Original rationale retained: exit-node validity is a Boolean projection of degradation — an exit is invalid iff every incident edge has capacity 0, which Dijkstra discovers automatically. Collapsed the plan's 5-function form. | `docs/multihazard_first_principles.md` §1, §5.1 |
...
| 12 | **JS-only hazard engine driven by YAML configs + 5-phase hook registry + declarative `result_extras` DSL.** (Three architectural revisions on 2026-05-24: v1 Python class-per-hazard → v2 Python hooks → v3 JS-only with 6-phase hooks → v3.1 JS-only with 5-phase hooks + result_extras DSL. See plan doc revision history.) Hazards are declared in `josh/config/hazards/{hazard_id}.yaml` (data source URL, zone tables, mobilization, dispositive flag, palette, labels, classify_strategy, hook overrides, runtime_extras), validated by a Python dataclass at build time (`agents/scenarios/hazard_config.py`), and emitted as JSON into `JOSH_DATA.hazard_configs[hazard_id]`. The generic engine (`static/hazard_engine.js`, ~400 lines) runs entirely in the browser; Python is reduced to its irreducible build-time role (OSMnx graph build, HCM capacity baking, spatial joins). **Five hook phases** split across both languages: `post_fetch` + `augment_graph` run in Python at pipeline time (GeoPandas/rasterio); `classify`, `degradation`, `egress_window` run in JS at runtime (per-project, per-scenario). `compute_delta_t` is un-hookable (audit-trail integrity). `build_result` is **engine-internal, NOT a hook** — variant-specific result fields are declared in `HazardConfig.result_extras` (declarative DSL) and interpreted by the engine. Supported `from:` types: `zone_lookup` (wildfire fhsz_haz_class), `containing_feature_attribute` (flood bfe_ft / tsunami eva_type / landslide burn_year), `all_containing_features` (dam dams_affecting), `nearest_feature_attribute` (gas operator/product), `in_zone` (tsunami in_cgs_tha), `literal`, `runtime_extras`, `engine_telemetry`. Wildfire + tsunami + landslide: zero hooks (pure result_extras). Flood: one Python hook (DEM/BFE augment) + one JS hook (degradation). Dam: two JS hooks (classify + egress_window). Gas: one Python hook (PIR buffer compute). Demo projects ship as YAML inputs only — no Python pre-baking of evaluation results; JS evaluates every project at sidebar load time. Phase D (v1→v2 translation) resolves automatically because seeded and custom projects use the same code path. Anti-divergence test pattern retired; replaced by hand-computed golden-file regression (`tests/golden/berkeley/*.expected.json`) with reviewer-signed provenance — stronger legal artifact than engine-to-engine comparison. No Stage 5 cutover needed (engine is production from Stage 1). Supersedes decision #6. | The Python evaluation engine duplicated work the JS engine already did — `wildland.py` and `whatif_engine.js` independently implemented the same Dijkstra + ΔT algorithm, kept in sync via test_vectors.json. Eliminating the duplication eliminates the divergence problem entirely. Python's irreducible role (GeoPandas/OSMnx/rasterio at build time) stays in Python; the runtime work that can already run in JS becomes JS-only. Golden-file regression replaces anti-divergence because there's nothing to diverge from. Phase D resolves as a free side effect. Architect review on 2026-05-24 validated the original Python-engine version with 7 fixes; second pass on 2026-05-24 identified that the JS-only formulation eliminates ~half of those concerns. Adopted with three picks: golden-file regression as the test pattern, no pre-commit performance measurement (acceptable Berkeley load-time estimate ~1.5 sec for 6 projects × 6 hazards), revise Stage 1 around JS engine. | `docs/plan-multihazard-stage-1-hooks.md` v3 |
...
| **Stage 0.5 Phase D (v1-result → v2-evaluation translation) DEFERRED** | When a user creates a custom project via `+ New`, the production `whatif_engine.js` populates `project.result` (legacy v1 shape) but NOT `project.evaluation.hazard_results` (v2 shape). The multi-hazard detail card reads off v2 only, so custom projects render with an empty bar chart + empty Hazards Evaluated list. **Decision (2026-05-18):** explicitly defer the v1→v2 translation until adapter contracts settle through Stages 1–9. Doing it now would almost certainly need to be redone once the real per-hazard shapes are pinned. Workaround: only the 4 pipeline-seeded + 2 mock-injected projects render correctly; new browser-created projects do not until Stage 1+ wires the translation. | 2026-05-18 |
| **Stage 1 architecture pivot: config + hook registry** ✅ DESIGN COMPLETE | After conversation-driven reassessment, the original class-per-hazard Stage 1 plan was replaced with a config-driven engine + 6-phase hook registry. Drafted in `docs/plan-multihazard-stage-1-hooks.md`; reviewed by `multihazard-architect` agent which returned "adopt with 7 modifications" verdict; all 7 fixes folded into the plan. Locked decision #6 marked superseded; new decision #12 added. | 2026-05-24 |
| **Stage 1 second pivot: JS-only engine + golden-file regression** ✅ DESIGN COMPLETE | After realizing the Python evaluation engine duplicates work the JS engine already does (kept in sync only by anti-divergence testing), Stage 1 was rescoped further: the engine moves entirely to JS (`static/hazard_engine.js`), Python narrows to build-time data prep, demo projects ship as YAML inputs evaluated by JS at sidebar load time, and anti-divergence is replaced by hand-computed golden-file regression with reviewer-signed provenance. Phase D (v1→v2 translation deferral) resolves automatically because seeded and custom projects use the same JS code path. No Stage 5 cutover needed — engine is production from Stage 1. Decision #12 rewritten to reflect JS-only architecture. Plan doc revised to v3. Ready for Stage 1 implementation dispatch. | 2026-05-24 |
| **Stage 1 third pivot: result_extras DSL replaces build_result hook** ✅ DESIGN COMPLETE | User pushed back on my "wildfire is structurally unique" framing of fhsz_haz_class. Re-examination showed every hazard has the same pattern (proximity to source → ranked severity → variant-specific raw fields preserved for audit). All hazards' variant-specific fields fit one of 8 generic data-extraction patterns (`zone_lookup`, `containing_feature_attribute`, `all_containing_features`, `nearest_feature_attribute`, `in_zone`, `literal`, `runtime_extras`, `engine_telemetry`). Promoted `build_result` from a hook phase to engine-internal driven by declarative `result_extras` YAML. Net effect: hook system drops from 6 phases to 5; wildfire truly has zero hooks; ~75% less code at the 6-hazard end state vs. hook-per-hazard approach; all per-hazard variant knowledge declarative in config. Plan doc revised to v3.1; decision #12 rewritten. Phase B Python (45/45) + JS (19/19) tests pass under the new architecture. | 2026-05-24 |
...
The original MVP stage order remains the umbrella plan — see `docs/plan-multihazard-mvp.md` for full detail. **Stage 1 architecture revised 2026-05-24** — see `docs/plan-multihazard-stage-1-hooks.md` for the active plan; the MVP doc's §4 (class-per-hazard) is superseded by locked decision #12.

1. **Stage 1 — JS-only hazard engine + YAML project loader.** `static/hooks.js` + `static/hazard_engine.js` + `static/transforms/*.js` + Python `hazard_config.py` (build-time loader) + `config/hazards/wildfire.yaml` + `tests/golden/berkeley/*.expected.json` + `tests/test_hazard_engine.js`. Demo projects ship as inputs only; JS evaluates at sidebar load. Phase D resolves automatically. Acceptance: golden tests pass for Berkeley's 4 seeded + 2 mock projects.
