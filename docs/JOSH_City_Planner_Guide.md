...
**Redesign the building.** For projects where the egress penalty is a significant contributor to ΔT — high-rise residential with a single stairwell and a single garage exit — the developer can add stairwells, widen stair widths, or provide multiple garage exits on different streets. The NFPA 101 Life Safety Code governs this calculation. The city engineer and fire department review this component. A building redesign that reduces the egress penalty reduces ΔT. The project is then re-analyzed.
